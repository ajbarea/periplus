...
## Tests
End-to-end tests run in [Playwright](https://playwright.dev/): they verify the app renders from `TRIP`, the maps-provider toggle persists, search filters the cards, and — by actually switching the browser offline and reloading — that the service worker serves the app with no network.
