...
Stages 1 (field editing) and 2 (card sections) shipped (see Done). Remaining:

- **The interactive / thin list sections** — weeks items and checklist items need a listener re-bind on re-render (`expandCurrentWeek`, `wireChecklists`); timeline rows are thin enough the corner controls want a different placement. The card-grid sections (directories, intel, contacts, anchors) are done via the `LISTS` registry — extending to these is mostly adding registry entries + their re-bind.
